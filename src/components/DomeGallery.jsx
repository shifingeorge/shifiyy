...
export default function DomeGallery({
  ...
  segments = DEFAULTS.segments, dragDampening = 2, openedImageWidth = '250px', openedImageHeight = '350px',
  ...
  const lastDragEndAt = useRef(0); const scrollLockedRef = useRef(false); const lockedRadiusRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      if (enlargedOverlay && frameRef.current && mainRef.current) {
        const frameR = frameRef.current.getBoundingClientRect(), mainR = mainRef.current.getBoundingClientRect();
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const cleanup = () => {
        animatingOverlay.remove(); originalTilePositionRef.current = null; if (refDiv) refDiv.remove();
  ...
  const openItemFromElement = useCallback(el => {
    ...
    overlay.style.transformOrigin = 'top left'; overlay.style.transition = `transform ${enlargeTransitionMs}ms ease, opacity ${enlargeTransitionMs}ms ease`;
    ...
    if (openedImageWidth || openedImageHeight) {
      const onFirstEnd = ev => {
        ...
        overlay.style.width = openedImageWidth || `${frameR.width}px`; overlay.style.height = openedImageHeight || `${frameR.height}px`;
